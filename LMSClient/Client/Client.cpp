...
{
	CWinSocket::initEnviornment();

	if (!CTCPSender::connect())
	{
	...
	while (1)
	{
		CDialogLogin loginDlg(&isSuccess, &user);
	...
	CMFCUtil::getActiveDoc()->init(user);
	CMFCUtil::getMainFrame()->init();
	CMFCUtil::getActiveView()->init();
	CTCPClient::getInstance()->startClient(user.getJson());
